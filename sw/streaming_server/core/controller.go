...
type IController interface {
	...
	HorusHandler(w http.ResponseWriter, r *http.Request, _ httprouter.Params)
	AlertsHandler(w http.ResponseWriter, r *http.Request, _ httprouter.Params)
...
func (c *Controller) isAuthenticated(r *http.Request, endpoint string) bool {
	...
	case "horus":
		if c.spec.Endpoints.Horus.Secret == "" {
			return true
		}
		return claims.Secret == c.spec.Endpoints.Horus.Secret
	case "alerts":
		if c.spec.Endpoints.Alerts.Secret == "" {
			return true
		}
		return claims.Secret == c.spec.Endpoints.Alerts.Secret
	case "start":
...
func (c *Controller) AlertsHandler(w http.ResponseWriter, r *http.Request, _ httprouter.Params) {
	/*if !c.isAuthenticated(r, "alerts") {
		w.WriteHeader(http.StatusForbidden)
		return
	}*/
	fmt.Println("alertHandler")
	resp, err := http.Get("http://192.168.1.10:8080/get-unresolved-alerts")
    if err != nil {
	...
	for key, camera := range result.Alerts {
		fmt.Println(key)
		dto = append(dto, &Alert{
			ID:		 camera.ID,
			Type:	 camera.Type,
	...
	w.Header().Add("Content-Types", "application/json")
	w.Write(b)
}
...
// StartStreamHandler is an HTTP handler for the POST /horus endpoint
func (c *Controller) HorusHandler(w http.ResponseWriter, r *http.Request, _ httprouter.Params) {
	/*if !c.isAuthenticated(r, "horus") {
		w.WriteHeader(http.StatusForbidden)
		return
	}*/

	resp, err := http.Get("http://192.168.1.10:8080/get-active-cameras")
    if err != nil {
        fmt.Println("No response from request")
    }
    defer resp.Body.Close()
    ...
    for _, rec := range result.Cameras {
        fmt.Println(rec.URL)


		index, knownStream := c.index[rec.URL]
		if knownStream {
			stream, ok := c.streams[index]
			if !ok {
				logrus.Errorf("Missing index for URI: %s | StartHandler", rec.URL)
				c.sendError(w, ErrUnexpected, http.StatusInternalServerError)
				return
			}
			// look for an alias since its known
		...
		if stream.Running {
			c.streams[id] = stream
			c.index[rec.URL] = id
			c.blacklist.Remove(rec.URL)
			// only add an alias if provided
			if len(rec.Alias) > 0 {
				c.alias[rec.Alias] = id
			}
		} else {
			c.blacklist.AddOrIncrease(rec.URL)
		}
		//c.sendStart(w, stream.Running, stream, rec.Alias)
	}
