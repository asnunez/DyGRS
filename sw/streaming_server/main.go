...
func main() {
	...
	if config.EndpointYML.Endpoints.Horus.Enabled {
		router.GET("/horus", controllers.HorusHandler)
		logrus.Infoln("horus endpoint enabled | MainProcess")
	}
	if config.EndpointYML.Endpoints.Alerts.Enabled {
		router.GET("/alerts", controllers.AlertsHandler)
		logrus.Infoln("alerts endpoint enabled | MainProcess")
	}
